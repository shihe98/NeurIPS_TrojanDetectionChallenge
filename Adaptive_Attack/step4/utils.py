...
def train_trojan_evasion(train_data, test_data, dataset, clean_model_path, attack_specification,
    ...
    real_cleam_path = '../evasive_dataset/reference_models' + clean_model_path[-17:]
    clean_model.load_state_dict(torch.load(real_cleam_path).state_dict())
    ...
    for epoch in range(num_epochs):
        ...
        for i, (bx, by) in enumerate(train_loader):
            ...
            nbx_trojan = []
            # nby_trojan = []
            negative_specs = generate_attack_specifications(np.random.randint(1e5), negative_batch_size, 'patch')
            negative_specs += generate_attack_specifications(np.random.randint(1e5), negative_batch_size, 'blended')
            for j in range(negative_batch_size):
                ...
                a = np.random.rand() * 5
                nbx_trojan_j, _ = insert_trigger(bx[j].unsqueeze(0), negative_specs[j + 100])
                nbx_trojan.append(nbx_trojan_j)
            ...
            with torch.no_grad():
                ...
                for kk in range(100):
                    nby_trojan[2 * kk] = by[kk]
                    nby_trojan[2 * kk + 1] = by[kk]
                    #nby_trojan[2*kk]=nby_trojan[2*kk]+0.15*by[kk]
                    #nby_trojan[2 * kk + 1]=nby_trojan[2*kk+1] + 0.15 * by[kk]
                #nby_trojan = nby_trojan + 0.11 * by
            ...
            # logit_sim_loss = (out1.detach() - out2).view(bx_logits.shape[0], -1).norm(p=1, dim=1).mean(0)
